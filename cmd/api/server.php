...
class RedisConnectionPool
{
	...
	public function __construct(string $host, int $port, ?string $auth, int $db, int $minConnections = 20, int $maxConnections = 100)
	{
		$this->host = $host;
		$this->port = $port;
		$this->auth = $auth;
	...
	private function createConnection(): ?\Redis
	{
		try {
			$redis = new \Redis();
			$redis->connect($this->host, $this->port, 0.2); // Timeout de conexão curto
			if ($this->auth) {
		...
		case 'POST':
			if ($uri === '/payments') {
				try {
					...
					go(function () use ($rawBody) {
						global $redisPool;
						$redis = $redisPool->get();
						if ($redis) {
							$redis->lpush('payment_queue', $rawBody);
							$redisPool->put($redis);
						}
					});
				} catch (Exception $e) {
